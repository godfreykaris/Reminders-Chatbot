    ...
    <div class="modal">
      ...
      {:else}
        <form on:submit|preventDefault={editGoal}>
          ...
          <label>
            Timezone:
            <select bind:value={selectedGoal.time_zone}>
          ...
          <label>
            Contact Choice:
            <select bind:value={selectedGoal.contact_choice}>
              <option value="SMS">SMS</option>
              <option value="WhatsApp">WhatsApp</option>
            </select>
          </label>
          <label>
            Report Frequency: <input type="number" bind:value={selectedGoal.report_frequency} on:input={handleInput} min={1} max={365} required />
          </label>
