...
class MessageSender:
    def send_message(self, recipient, message, message_type, credentials=None):
        ...
        try:
            if message_type == 'sms':
                if credentials:
                    message_sender = TwilioSMSSender(credentials_file_path='', credentials=credentials)
                else:
                    message_sender = TwilioSMSSender("config.json")
                sent = message_sender.send_sms(recipient, message)
            elif message_type == 'whatsapp':
                if credentials:
                    message_sender = TwilioWhatsAppSender(credentials_file_path='', credentials=credentials)
                else:
